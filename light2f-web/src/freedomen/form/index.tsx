...
function isArrayUpdateKey({ prop, $preData, $data }: any = {}) {
    const preValue = $preData[prop]
    const currentValue = $data[prop]
    let isUpdateKey = false

    if (Array.isArray(preValue) && Array.isArray(currentValue) && preValue.length === currentValue.length) {
        for (let i = 0; i < currentValue.length; i ++) {
            if (!(preValue[i].$index && preValue[i].key) && currentValue[i].$index && currentValue[i].key) {
                isUpdateKey = true
            } else {
                isUpdateKey = false
                break
...
const FForm: React.ForwardRefRenderFunction<FFormRef, IFormProps> = (props, ref) => {
    ...
    const innerEvent = useCallback((params: any) => {
        if (params.type === changeType && params.prop && isArrayUpdateKey(params.column)) {
