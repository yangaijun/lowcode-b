...
import hotelVipService from "services/HotelVip"
import styles from "./index.module.less"

export default function HotelVip() {
    ...
    const loadData = useCallback(() => {
        setLoading(true)
        hotelVipService.search(searchParams).then((res) => {
            setLoading(false)
            if (res.code === 0) {
                //此处的数据结构可能不同，需要修改
                setPagination({
    ...
    const tableEvent = (params) => {
        if (params.prop === "edit" && params.type === "click") {
            ...
            hotelVipService.delete(params.row).then((res) => {
    ...
    const getDialogForm = (formData) => {
        return (
            <Form
                ...
                data={formData}
                columns={[
                    { type: "input", prop: "vipName", label: "名称", rule: "required" },
                    { type: "input", prop: "vipPhone", label: "手机号", rule: "phone" },
                ]}
            />
        )
    }
    ...
    return (
        <div className={"freedomen-page"}>
            ...
            <Table
                ...
                columns={[
                    { type: "text", prop: "vipName", label: "名称", value: "text" },
